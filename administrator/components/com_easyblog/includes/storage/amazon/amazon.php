...
class EasyBlogStorageAmazon implements EasyBlogStorageInterface
{
	...
	public function replaceMediaUrlToJoomla($content)
	{
		...
		$content = JString::str_ireplace('"' . $amazonUrl, '"', $content);
		return $content;
	}
	...
	public function replaceMediaUrlToAmazon($relativePath, $content)
	{
		...
		$content = JString::str_ireplace('"' . $relativePath, '"' . $newImageSrc, $content);
		...
		// if ($searches) {
		// 	$content = JString::str_ireplace($searches, $replacements, $content);
		// }
