...
class EasyBlogClientTwitter extends EasyBlogTwitterOAuth
{
	...
	public function uploadMedia(EasyBlogPost &$post)
	{
		...
		$postImage = $post->getImage('thumbnail', false, true);

		if (!$postImage) {
			$postImage = EB::getPlaceholderImage();
		}
	...
	public function formatMessage(EasyBlogPost &$post, EasyBlogTableOAuth &$oauth)
	{
		...
		if (EBString::stristr($content, '{introtext}') !== false) {
			...
			$introText = EBString::substr(strip_tags($introText), 0, 50) . JText::_('COM_EASYBLOG_ELLIPSES');
			$replace[] = $introText;
		}
		...
		if (EBString::stristr($content, '{link}') !== false) {
			...
			$length = 22;
			$url = 'index.php?option=com_easyblog&view=entry&id=' . $post->id;
			...
			$content = EBString::substr($content, 0, 136) . JText::_('COM_EASYBLOG_ELLIPSES');
		}
