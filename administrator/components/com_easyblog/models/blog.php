...
class EasyBlogModelBlog extends EasyBlogAdminModel
{
	...
	public function __construct()
	{
		...
		$limitstart = $this->input->get('limitstart', 0, 'raw');
...
>>>>>>> master
		...
		$limitstart = $this->input->get('limitstart', 0, 'REQUEST');
...
>>>>>>> master
	...
	public function addBlogSubscription($blogId, $email, $userId = '0', $fullname = '')
	{
		...
		if ($acl->get('allow_subscription') || (empty($my->id) && $config->get('main_allowguestsubscribe'))) {
			...
			if ($state) {
				...
				$template->targetlink = EBR::getRoutedURL('index.php?option=com_easyblog&view=entry&id=' . $blogId, false, true);
	...
	public function getRelatedPosts($id, $max = 0)
	{
		...
		$query[] = 'ORDER BY c.'. $db->quoteName($sortItem) . ' ' . $orderItem;
