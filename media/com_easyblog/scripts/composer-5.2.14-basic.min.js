...
c.each(function(){{var c=a(this);b.getBlockType(c)}b.initBlock(c)})},"{self} composerDocumentBlur":function(){return b.refocusing?(b.refocusing=!1,void 0):(b.deactivateBlock(),void 0)},"{self} click":function(c,d){var e=b.view().parent().hasClass("open");if(e){var f=a(d.target);f.is("[data-toolbar-blocks]")||f.parents("[data-eb-composer-blocks]").length>0||b.view().parent().removeClass("open")}},"{self} keyup":function(a,c){var d=b.view().parent().hasClass("open");d&&27==c.keyCode&&b.view().parent().removeClass("open")
},"{editableContent} focusin":function(a,c){c.focusHandled||b.refocusing||t.document.isLegacy()||(b.refocusing=!0,c.focusHandled=!0,setTimeout(function(){var c=(b.workarea(),t.editor.selection.get()),d=c.focusNode;if(!d)return b.refocusing=!1,void 0;var e=b.block.of(d);if(!e.length)return b.refocusing=!1,void 0;if(!e.hasClass(h)){e.addClass(h);try{if(e.hasClass("active")){var f=b.getBlockHandler(e);f.refocus(e)}else b.activateBlock(e)}catch(g){}e.removeClass(h),setTimeout(function(){b.refocusing=!1},100)}},1))
}}}),b.resolve()})}),EasyBlog.module("composer/blocks/dimensions",function(){var b=this,c=function(a){return a.toString().match("%")?"%":"px"};EasyBlog.require().script("composer/vendors/nouislider").done(function(a){EasyBlog.Controller("Composer.Blocks.Dimensions",{elements:["[data-eb-block-dimensions-{field}]","[data-eb-block-dimensions-field-container] [data-eb-{numslider-toggle|numslider-widget|numslider-value|numslider-input|numslider-units|numslider-unit|numslider-current-unit}]"],defaultOptions:a.extend({},EBD.selectors)},function(b,d,e,f,g,h){return{init:function(){g=b.blocks,f=g.composer,h=a()
...
}b.populate()},deactivate:function(c){b.activateKeys();var d=b.getContent(c);if(d=a.trim(d),0==d.length||" "==d){var e=f.getBlockContent(c),h=b.contentWrapper.inside(e),i=h.find("> p");1===i.length&&i.html(g.data.default)}},construct:function(){var b=f.createBlock("text"),c=f.getBlockContent(b);return c.html(g.data.content),b},reconstruct:function(c){var d=f.getBlockContent(c),e=b.blockWrapper.inside(d);e.length||(e=a(g.blockWrapper).append(d[0].childNodes).appendTo(d));var h=b.contentWrapper.inside(d);if(!h.length){var i=[];
...
}),a(this).on("blur",function(){j.removeClass("input-focused")})}),b.trigger("composerReady")},isPublished:function(){return b.publishedField().val()==g},isDraft:function(){return b.publishedField().val()==e},"{self} composerDocumentReady":function(){h.removeClass("is-loading")},"{self} composerSubmitLock":function(){h.addClass("is-submit-lock")},"{self} composerSubmitUnlock":function(){setTimeout(function(){h.removeClass("is-submit-lock")},2e3)},installPlugins:function(c){a.each(c,function(a,c){b.addPlugin(c)
})},settings:{get:function(a){return d.find("input[name='"+a+"']").val()},set:function(a,c){return d.find("input[name='"+a+"']").val(c),b.trigger("composerSettingsChange",[a,c]),c}},getPreviewNamespace:function(){return"post-preview-"+c.authorId+"-"+b.getPostUid()},getPostId:function(){return h.attr("data-post-id")},getPostUid:function(){return h.attr("data-post-uid")},getRevisionId:function(){return b.getPostUid().split(".")[1]},getDoctype:function(){return h.attr("data-post-doctype")},views:{show:function(a){b.trigger("composerViewShow",[a])
},hide:function(a){b.trigger("composerViewHide",[a]);var c=b.toolbarViews("[data-type="+a+"]");c.length&&c.removeClass("open"),b.views.show("document")}},keepalive:{timer:null,start:function(a){var d=b.keepalive,a=a||c.keepalive.interval;d.stop(),1>a||(d.timer=setTimeout(function(){d.run(a)},a))},run:function(a){EasyBlog.ajax("site/views/composer/keepAlive").always(function(){b.keepalive.start(a)})},stop:function(){clearTimeout(b.keepalive.timer)}},autosave:{timer:null,counter:0,start:function(a){var a=a||c.autosave.interval;
...
})},"{submitApproval} click":function(a){b.disableLeavePrompt(),a.addClass("is-loading"),b.publishedField().val(f),b.save({autosave:0,isapply:0})},"{approvePost} click":function(a){b.publishedField().val(g),a.addClass("is-saving is-loading"),b.disableLeavePrompt(),b.save({autosave:0,isapply:0}).done(function(){}).always(function(){a.removeClass("is-saving")})},"{rejectPost} click":function(){b.publishedField().val(e),b.disableLeavePrompt(),b.save({autosave:0,isapply:0}).done(function(){})},"{publish} click":function(a){h.hasClass("is-submit-lock")||(b.previewField().val(0),b.publishedField().val(g),b.disableLeavePrompt(),a.addClass("is-loading"),b.save({autosave:0,isapply:0}))
},"{updatePost} click":function(a){h.hasClass("is-submit-lock")||(b.publishedField().val()==e&&(b.previewField().val(0),b.publishedField().val(g)),b.disableLeavePrompt(),a.addClass("is-loading"),b.save())},"{unpublishPostButton} click":function(){b.publishedField().val(i),b.save({autosave:0,isapply:0})},"{trashPost} click":function(){b.trash()},"{draftContinue} click":function(){b.disableLeavePrompt()},"{draftDiscard} click":function(){EasyBlog.ajax("site/views/composer/discardDraft",{id:b.getPostId()}).done(function(a){b.setMessage({type:"success",message:a}),j.removeClass("warning-draft"),b.draftItems().remove()
...
"post"==e&&(g=b.getCurrentPostUri());var h=b.currentUri;h!==g&&(b.selection.clear(),b.setActivePlace(g),a.addClass("is-loading"),c.mobile&&b.frame().removeClass("is-places-open"),b.navigate(g).done(function(){a.removeClass("is-loading").addClass("is-active")}))},"{loadMore} click":function(a){var d=a.data("page"),e=b.getKey("flickr"),f=a.find("[data-loadmore-container]");f.addClass("is-loading"),b.selection.clear(),EasyBlog.ajax("site/views/mediamanager/loadmore",{key:e,page:d}).done(function(c){a.remove(),b.groupList().append(c.contents)
...
if(b.selectionCounter().html(g),0>=g)return b.actions().removeClass("show-insert show-gallery show-permalink"),void 0;var h=e.document.isLegacy(),i=b.selection.getLastItem(),j=i.meta,k=f.meta;if("folder"!=k.type||h){if("folder"==k.type&&h)return b.actions().addClass("show-gallery"),void 0;g>=1&&b.actions().addClass("show-insert"),g>1&&b.selection.canCreateGallery()?b.actions().addClass("show-gallery"):b.actions().removeClass("show-gallery"),h&&"pdf"==k.type&&k.id==j.id&&"pdf"==j.type?b.actions().addClass("show-permalink"):h&&"pdf"==j.type?b.actions().addClass("show-permalink"):b.actions().removeClass("show-permalink")
