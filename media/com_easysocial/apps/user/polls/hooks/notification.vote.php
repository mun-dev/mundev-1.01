...
class SocialUserAppPollsHookNotificationVote
{
...
>>>>>>> master
		...
		if ($item->cmd == 'polls.vote.item') {
			...
			if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				...
				$langString = ES::string()->computeNoun('APP_USER_POLLS_USER_VOTED_ON_YOUR_POLL', count($users));
				$item->title = JText::sprintf($langString, $names, $poll->title);
