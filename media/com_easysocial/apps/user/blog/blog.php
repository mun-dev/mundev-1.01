...
class SocialUserAppBlog extends SocialAppItem
{
	...
	public function isItemViewable($action, $context, $verb, $uid)
	{
		...
		if (!$privacy->validate('easyblog.blog.view', $uid, 'blog', $post->created_by)) {
			return false;
		}
	...
	*/
	...
	private function prepareNewBlogStream(&$item)
	{
		...
		if (!is_null($useEBTruncation)) {
			$content = $this->getContentWithoutMedia($post);
	...
	public function onAfterCommentSave(&$comment)
	{
		...
		$emailOptions = array(
			'template' => 'apps/user/blog/new.comment',
			'actor' => $actor->getName(),
		...
		if ($comment->created_by != $post->created_by) {
			ES::notify('blog.comment', array($post->created_by), $emailOptions, false);
		}
	}
	 ...
	 * @access  public
	 */
	public function getContentWithoutMedia($post)
	{
		// We need to get raw content (striptags = false) so that we can strip media properly
		$content = $post->getIntro(false, false, 'intro', null, array('triggerPlugins' => false));
		$media = $post->getMedia();
		$post->text = $content;
